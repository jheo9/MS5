...
namespace sdds
{
	Perishable::Perishable() : m_expiry_date()
	{
	}
	...
	std::ostream& Perishable::display(std::ostream& ostr) const 
	{
		if (m_status)
		{
			...
			{
				Item::display(ostr);
				if (m_handling_instructions != nullptr && strlen(m_handling_instructions) > 0 && strcmp(m_handling_instructions, "\n") != 0) 
				{
					ostr << '*';
				}
				else
			...
			{
				cout << endl << "Perishable ";
				Item::display(ostr);
				ostr << "Expiry date: " << m_expiry_date << endl;
				if (m_handling_instructions != nullptr && strlen(m_handling_instructions) > 0 && strcmp(m_handling_instructions, "\n") != 0) 
				{
	...
	std::ifstream& Perishable::load(std::ifstream& ifstr) 
	{
		...
		if (temp != nullptr)
		{
			m_handling_instructions = new char[strlen(temp) + 1];
	...
	{ 
		return perishable_item.display(ostr);
	};
	std::istream& operator>>(std::istream& istr, Perishable& perishable_item) 
	{
		return perishable_item.read(istr);
	};
}
